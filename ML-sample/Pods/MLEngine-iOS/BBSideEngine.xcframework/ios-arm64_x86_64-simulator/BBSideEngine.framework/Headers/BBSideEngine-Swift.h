...
#ifndef BBSIDEENGINE_SWIFT_H
#define BBSIDEENGINE_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="BBSideEngine",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
# pragma pop_macro("any")
#endif

#if defined(__OBJC__)
typedef SWIFT_ENUM(NSInteger, BBMode, open) {
  BBModeSandbox = 0,
  BBModeProduction = 1,
};
...
SWIFT_CLASS("_TtC12BBSideEngine10BBResponse")
@interface BBResponse : NSObject
@property (nonatomic) BOOL success;
@property (nonatomic) enum BBSideOperation type;
@property (nonatomic, copy) NSDictionary<NSString *, id> * _Nullable payload;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

enum BBTheme : NSInteger;
@class UIColor;
...
@property (nonatomic, strong) UIColor * _Nonnull swipeToCancelTextColor;
@property (nonatomic, strong) UIColor * _Nonnull swipeToCancelBackgroundColor;
...
- (void)configureWithAccessKey:(NSString * _Nonnull)accessKey mode:(enum BBMode)mode theme:(enum BBTheme)theme;
@end
...
- (void)sendSMSWithContacts:(NSArray * _Nonnull)contacts;
- (void)sendEmailToEmail:(NSString * _Nonnull)toEmail;
...
typedef SWIFT_ENUM(NSInteger, BBSideEngineMode, open) {
  BBSideEngineModeTest = 0,
  BBSideEngineModeLive = 1,
};

typedef SWIFT_ENUM(NSInteger, BBSideOperation, open) {
  BBSideOperationNone = 0,
  BBSideOperationConfigure = 1,
  BBSideOperationStart = 2,
  BBSideOperationStop = 3,
  BBSideOperationSms = 4,
  BBSideOperationEmail = 5,
